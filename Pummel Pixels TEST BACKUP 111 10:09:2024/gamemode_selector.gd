...
@onready var practice_button = $GamemodeButtons/PracticeButton
...
zombies_button, wanted_button, juggernaut_button, practice_button]
...
func _process(_delta):
	...
	if current_state == state.MAPSELECT:
		...
		elif practice_button.has_focus():
			gamemode_label.text = "PRACTICE"
			gamemode_description_label.text = "PRACTICE WITH DUMMIES!"
			gamemode_description_label.add_theme_color_override("font_shadow_color", Color("#c97f11"))
			gamemode_label.add_theme_color_override("font_shadow_color", Color("#c97f11"))
...
func _on_practice_button_pressed():
	...
	GameSettings.current_gamemode = GameSettings.GAMEMODE.PRACTICE
	switch_to_map_selection()
