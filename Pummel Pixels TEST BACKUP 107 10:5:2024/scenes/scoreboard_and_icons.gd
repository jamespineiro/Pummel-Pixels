...
func _ready():
	...
	if GameSettings.current_gamemode == GameSettings.GAMEMODE.PRACTICE:
		...
		GameSettings.game_lives = 9999
		goal_score = 9999
		match_timer.wait_time = 9999999
		label.text = "INF"
		label.add_theme_font_size_override("font_size", 240)
		label.global_position.x -= 2
		label.global_position.y -= 7
	else:
		GameSettings.game_lives = 3
...
func _process(_delta):
	...
	if (countdown.visible or GameSettings.current_gamemode == GameSettings.GAMEMODE.PRACTICE) and not game_just_finished:
		if countdown.text == "1" and GameSettings.current_gamemode != GameSettings.GAMEMODE.PRACTICE:
			bell_sound.play()
		if countdown_timer.time_left > 4 and GameSettings.current_gamemode != GameSettings.GAMEMODE.PRACTICE:
			countdown.text = "3"
		elif countdown_timer.time_left >= 2 and GameSettings.current_gamemode != GameSettings.GAMEMODE.PRACTICE:
			countdown.text = str(int(countdown_timer.time_left-1))
		elif (countdown_timer.time_left < 2 and countdown_timer.time_left > 1) or GameSettings.current_gamemode == GameSettings.GAMEMODE.PRACTICE:
